...
public class ModConfiguredFeatures {
    ...
    public static void bootstrap(BootstapContext<ConfiguredFeature<?, ?>> context) {
        ...
        List<OreConfiguration.TargetBlockState> overworldnaturestavess = List.of(
                OreConfiguration.target(stoneReplaceabeles, ModBlocks.MINERAL_ORE.get().defaultBlockState()));

        register(context, OVERWORLD_MINERAL_ORE_KEY, Feature.ORE, new OreConfiguration(overworldnaturestavess, 9));
    }

    public static ResourceKey<ConfiguredFeature<?, ?>> registerKey(String name) {
        return ResourceKey.create(Registries.CONFIGURED_FEATURE, new ResourceLocation(NatureStavesMod.MOD_ID, name));
    }
